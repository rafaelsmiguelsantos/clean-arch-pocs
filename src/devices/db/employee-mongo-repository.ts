...
export class EmployeeMongoRepository implements EmployeeRepository {
	...
	private employeeToDTO(employee: Employee, hashedPassword): EmployeeData {
		return {
			...
			companyId: new ObjectId(employee.getCompanyId()),
			...
			userId: new ObjectId(employee.getUserId())
		};
	}

	private employeeDataToEmployeeResponseDTO(employeeData: EmployeeData): EmployeeResponseDTO {
		let id: string;
		if (employeeData._id instanceof ObjectId) {
			id = employeeData._id.toHexString();
		} else {
			id = employeeData._id;
		}

		let companyId: string;
		if (employeeData.companyId instanceof ObjectId) {
			companyId = employeeData.companyId.toHexString();
		} else {
